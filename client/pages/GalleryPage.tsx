...
function GalleryTile({ entry }: { entry: Entry }): ReactElement {
  return (
    <figure
      className="card"
      style={{ margin: 0, padding: 8, overflow: 'hidden' }}
    >
      {entry.imageUrl ? (
        <img
          src={entry.imageUrl}
      ...
      <figcaption style={{ marginTop: 8 }}>
        <div style={{ fontWeight: 700, fontFamily: 'var(--orn-font)' }}>
          {entry.title}
        </div>
        {entry.funFact && (
...
function EntryRow({ entry }: { entry: Entry }): ReactElement {
  return (
    <div className="card" style={{ marginBottom: 0 }}>
      ...
      >
        <h2
          style={{ margin: '.1em 0', fontFamily: 'var(--orn-font)', flex: 1 }}
        >
          {entry.title}
        </h2>
        {entry.link && <ExternalLink href={entry.link} />}
...
export default function GalleryPage({
  ...
  const label =
    CORNERS.find((c) => c.key === corner)?.label ??
    ENTRY_CORNER_LABELS[corner] ??
    corner;
