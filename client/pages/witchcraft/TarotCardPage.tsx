...
export default function TarotCardPage({ cardId }: { cardId: string }): ReactElement {
  const card = tarotById(cardId);

  if (!card) {
    return (
      <Win9xWindow title="tarot — 404" className="article-win" bodyClassName="doc-body">
        <h1 className="article">Card not found</h1>
        <p className="note">No tarot card matches “{cardId}”.</p>
        <Link to="witchcraft/tarot" params={{}} className="tbtn" style={{ marginTop: 12 }}>
  ...
  return (
    <Win9xWindow title={`${card.id}.card`} className="article-win" bodyClassName="doc-body">
      ...
      <div
        ...
        style={{ gridTemplateColumns: 'repeat(auto-fit, minmax(260px, 1fr))', gap: 12, marginTop: 16 }}
      >
