    ...
    const run = async () => {
      try {
        const [tRes, aRes] = await Promise.all([
          apiPost<{ thoughts: RandomThought[] }>('listRandomThoughts', {
            limit: 1,
          }),
    ...
    <>
      <AnimateIn>
        <div className="announce">
          <span>
            ✦ welcome to 317010.xyz ✦ best viewed in Netscape Navigator @
            1024×768 ✦ now with 100% more chaos ✦ Cosmoo the loaf-knight
            approves ✦ sign the guestbook ✦ mind the scanlines ✦ tea is brewing
            ✦
          </span>
        </div>
      </AnimateIn>
      ...
      {isEmpty && (
        <AnimateIn>
          ...
          >
            ...
            <h2>no posts yet</h2>
            <p className="note">
              this little corner of the internet is still warming up — no random
              thoughts and no published articles just yet. brew some tea and
              check back soon ✦
            </p>
          </Win9xWindow>
        </AnimateIn>
      ...
      <AnimateIn>
        <Win9xWindow
          title="★ random_thoughts.txt — Notepad"
          className="featured"
        >
          ...
          {thought ? (
            <>
              ...
              <div className="time">
                {new Date(toMs(thought.created)).toLocaleString()}
              </div>
              <p>{thought.body}</p>
            </>
          ) : (
            <>
              <h2>a little corner of the internet</h2>
              <div className="time">
                {loaded ? 'no thoughts yet' : 'loading…'}
              </div>
              <p>
                {loaded
      ...
      <AnimateIn>
        <Win9xWindow
          title="recent_articles.dir — Explorer"
        ...
        >
          ...
          {loaded && articles.length === 0 && (
            <p className="note">
              no published articles yet — the corners await their first words.
            </p>
          )}
          {articles.length > 0 && (
            <div
              ...
              {articles.map((a) => (
                <AnimateIn key={a._id}>
                  ...
                  >
                    ...
                    {a.excerpt && (
                      <p style={{ margin: '.4em 0 0' }}>{a.excerpt}</p>
                    )}
