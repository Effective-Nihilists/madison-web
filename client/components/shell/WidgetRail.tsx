...
function WidgetBox({
  ...
  useEffect(() => { setLiveH(height); }, [height]);
  ...
  const sized: CSSProperties = liveH !== null ? { height: liveH, overflow: 'auto' } : {};
...
export default function WidgetRail({
  ...
  useEffect(() => {
    let alive = true;
    const apply = (count: number): void => { if (alive) setVisitors(count); };
    ...
    if (already) {
      void apiPost<{ count: number }>('getVisitCount', {}).then((r) => { apply(r.count); }).catch(() => { /* offline */ });
    } else {
      sessionStorage.setItem(VISIT_SESSION_KEY, '1');
      void apiPost<{ count: number }>('recordVisit', {}).then((r) => { apply(r.count); }).catch(() => { /* offline */ });
    }
    return () => { alive = false; };
  }, []);
  ...
  const { itemProps } = useDragReorder(ordered.map((w) => w.id), (next) => { save({ widgetOrder: next }); });
      ...
      {ordered.map((w) => (
        <AnimateIn key={w.id}>
          <WidgetBox
            ...
            onResize={(h) => { save({ boxSizes: { ...config.boxSizes, [w.id]: { w: null, h } } }); }}
          >
            {w.render()}
          </WidgetBox>
        </AnimateIn>
