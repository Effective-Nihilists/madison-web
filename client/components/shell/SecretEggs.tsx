...
>(function SecretEggs({ milkdrop }, ref) {
  ...
  function secretFound(msg: string): void {
    toast(`✦ you found a secret! ${msg}`);
  }
  ...
  function rainDoodles(count: number): void {
    for (let i = 0; i < count; i++) {
      ...
      s.style.color =
        DOODLE_COLORS[Math.floor(Math.random() * DOODLE_COLORS.length)] ??
        '#c79a3a';
  ...
  function runCosmoo(): void {
    ...
    setTimeout(() => {
      if (cosmooRef.current) cosmooRef.current.style.display = 'none';
    }, 2700);
  }

  function openRoom(): void {
    setRoomOpen(true);
    secretFound('the secret room');
  }
  ...
  useImperativeHandle(
    ...
    (): SecretEggsHandle => ({
      rainDoodles: (count) => {
        ...
        secretFound('the autumn rains');
      },
      runCosmoo: () => {
        runCosmoo();
        secretFound('a passing cat');
      },
      openRoom,
    }),
    [],
  ...
  useEffect(() => {
    ...
    hot.style.cssText =
      'position:fixed;right:0;bottom:0;width:34px;height:34px;z-index:50;cursor:crosshair';
    ...
    return () => {
      hot.removeEventListener('click', onClick);
      hot.remove();
    };
  ...
  useEffect(() => {
    ...
    function chaosMode(): void {
      ...
      setTimeout(() => {
        milkdrop.current?.setOpacity(0.5);
      }, 5000);
    }
    ...
    <>
      ...
      >
        ...
        >
          ...
          <p>
            you found the hidden door behind the wordmark. welcome, ghost. there
            is tea here, and the cat approves of you. tell no one. (click
            anywhere to leave)
          </p>
        </div>
      </div>
